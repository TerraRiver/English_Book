...
const BATCH_SYSTEM_PROMPT = `你是一个英语词典助手。给定一组英文单词或短语（JSON 数组），为其中每一个词条生成详细信息，仅以 JSON 对象输出，不要包含任何解释性文字或 markdown 代码块标记。JSON 结构如下：
{
  "words": {
    "<词条原文，必须与输入中的文本逐字符一致>": {
      "phonetic_uk": "英式音标，如 /wɜːd/，没有则省略",
...
必须为输入数组中的每一个词条都生成一个对应条目，键名必须与输入文本完全一致，不要增删或改写词条文本。senses 至少 1 条，覆盖常见词性和释义；examples 提供 1-2 条例句；variants 只在该词确实存在规则或不规则变体时提供。`

export async function lookupWordsBatch(
  terms: string[],
  settings: LlmSettings
): Promise<Map<string, WordDetail | null>> {
  if (!settings.apiKey) {
    throw new Error("请先在设置中填写 API Key")
  }
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => "")
    throw new Error(`批量查词请求失败 (${res.status}): ${text.slice(0, 300)}`)
  }
  ...
  let parsed: { words?: Record<string, WordDetail> }
  try {
    parsed = JSON.parse(content)
  } catch {
    throw new Error("模型返回的内容不是合法 JSON")
  ...
  const result = new Map<string, WordDetail | null>()
  const words = parsed.words ?? {}
  for (const term of terms) {
    const detail = words[term]
    if (detail && Array.isArray(detail.senses) && detail.senses.length > 0) {
      detail.variants ??= []
      detail.examples ??= []
      result.set(term, detail)
    } else {
